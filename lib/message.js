...
function Message (newMsg) {
  ...
  if (newMsg.readable && newMsg.pipe) {
    return Message._bufferMessageStream(newMsg);
  }
...
Message.prototype          = {
  ...
  // Promise pass through functions
  //then: function(){
  //  return this.promise.then.call(q(this.promise), arguments);
  //},
  //done: function(){
...
;
Message.prototype.toString = Message.prototype.inspect; // Alias

/**
 * Buffer to get full stream before acting on it.
 *
 * @param stream
 * @return {Message}
 * @private
 */
Message._bufferMessageStream = function bufferMessageStream (stream) {
  ...
  stream.on('end', function () {
    ...
    Object.getOwnPropertyNames(bufferedMsg).forEach(function (k) {
